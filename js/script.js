...
btnCopy.addEventListener('click', () => {
    navigator.clipboard.writeText(copyContent).then(() => {
        const noticeBody = document.getElementById('notice')
        const notice = document.createElement('div')
        notice.classList.add('notice__body')
        notice.innerHTML = 'The code has been copied'
...
const renderShape = () => {
    if (radioShadow.checked) {
        ...
        currentShadow = `inset -${sizeDistance}px -${sizeDistance}px ${
            sizeDistance * 2
        }px ${currentColor[0]},
      inset ${sizeDistance}px ${sizeDistance}px ${sizeDistance * 2}px ${
            currentColor[2]
        }`
    } else {
        // Проверка на направление света должна быть здесь
        currentShadow = `-${sizeDistance}px -${sizeDistance}px ${
            sizeDistance * 2
        }px ${currentColor[0]},
      ${sizeDistance}px ${sizeDistance}px ${sizeDistance * 2}px ${
            currentColor[2]
        }`
    }
...
// Generate CSS
btnGenerate.onclick = () => {
    document.getElementById('output-width').textContent = rangeSize.value
    document.getElementById('output-height').textContent = rangeSize.value // Лишний код, можно зарефакторить
    document.getElementById('output-border-radius').textContent =
        rangeRadius.value
    document.getElementById('output-background').textContent = currentColor[1]
    document.getElementById('output-shadow').textContent = currentShadow
    output.style.display = 'flex'

    copyContent = `
    width: ${rangeSize.value}px;
    height: ${rangeSize.value}px;
    border-radius: ${rangeRadius.value}px;
